...
import { Search, SlidersHorizontal, ShoppingBag, X, Star } from "lucide-react";
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="flex-1">
          ...
          <Link to={`/product/${product._id}`}>
            <h3 className="font-display text-xl text-obsidian-900 hover:text-gold-700 transition-colors leading-tight mb-2">
              {product.name}
            </h3>
          </Link>
          <p className="text-obsidian-500 text-xs line-clamp-2 mb-3">{product.description}</p>
        </div>

        <div className="flex items-center justify-between mt-auto pt-3 border-t border-obsidian-100">
          <div>
            <p className="font-display text-2xl text-obsidian-900">₹{product.price?.toLocaleString("en-IN")}</p>
...
export default function Shop() {
  ...
  return (
    <div className="min-h-screen bg-cream">
      {/* Header */}
      <div className="bg-obsidian-950 pt-10 pb-16 px-4">
        <div className="max-w-7xl mx-auto">
          <p className="section-tag text-gold-400 mb-2">Our Collection</p>
          <h1 className="font-display text-5xl text-white mb-6">All Jewellery</h1>
          ...
          <div className="relative max-w-xl">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3.5 rounded-2xl bg-obsidian-800 border border-obsidian-700 text-white placeholder:text-obsidian-400 focus:outline-none focus:ring-2 focus:ring-gold-500 font-body text-sm"
            />
            {search && (
              <button onClick={() => setSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 p-1 hover:text-white text-obsidian-400">
                <X className="w-4 h-4" />
              </button>
            )}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 -mt-6">
        ...
        {loading ? (
          ...
          <motion.div
            layout
            className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 pb-20"
          >
            <AnimatePresence>
